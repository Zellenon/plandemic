...
class BoardGame:
    def __init__(self, board_file: str, doors_file: str, num_agents: int):
        ...
        self.stop_button_rect = pygame.Rect(
            10, self.height * self.CELL_SIZE + 10, 120, 30
        )
        ...
        shroom_texture = pygame.image.load(
            "assets/ingredients/shroom.png"
        ).convert_alpha()
        berry_texture = pygame.image.load(
            "assets/ingredients/berry.png"
        ).convert_alpha()
        self.ingredients_sprites = {
            "eye": pygame.transform.scale(
                eye_texture, (self.CELL_SIZE, self.CELL_SIZE)
            ),
            "shroom": pygame.transform.scale(
                shroom_texture, (self.CELL_SIZE, self.CELL_SIZE)
            ),
            "berry": pygame.transform.scale(
                berry_texture, (self.CELL_SIZE, self.CELL_SIZE)
            ),
        }
    ...
    def load_textures(self):
        ...
        try:
            ...
            cauldron_texture = pygame.transform.scale(
                # Scale to cell size
                cauldron_texture,
            ...
            stone_texture = pygame.transform.scale(
                stone_texture, (self.CELL_SIZE, self.CELL_SIZE)
            )
            mossy_texture = pygame.transform.scale(
                mossy_texture, (self.CELL_SIZE, self.CELL_SIZE)
            )
            wood_texture = pygame.transform.scale(
                wood_texture, (self.CELL_SIZE, self.CELL_SIZE)
            )
            wood_texture2 = pygame.transform.scale(
                wood_texture2, (self.CELL_SIZE, self.CELL_SIZE)
            )
            cobble_texture = pygame.transform.scale(
                cobble_texture, (self.CELL_SIZE, self.CELL_SIZE)
            )
            wood_texture3 = pygame.transform.scale(
                wood_texture3, (self.CELL_SIZE, self.CELL_SIZE)
            )
            grass_texture = pygame.transform.scale(
                grass_texture, (self.CELL_SIZE, self.CELL_SIZE)
            )
            dirt_texture = pygame.transform.scale(
                dirt_texture, (self.CELL_SIZE, self.CELL_SIZE)
            )
            water_texture = pygame.transform.scale(
                water_texture, (self.CELL_SIZE, self.CELL_SIZE)
            )
    ...
    def draw_buttons(self):
        ...
        color = (
            (200, 50, 50) if not self.paused else (50, 200, 50)
        )  # Red for running, green for paused
        pygame.draw.rect(self.screen, color, self.stop_button_rect)
        font = pygame.font.Font(None, 24)
        text = "Pause" if not self.paused else "Resume"
        text_render = font.render(text, True, (255, 255, 255))
        self.screen.blit(
            text_render, text_render.get_rect(center=self.stop_button_rect.center)
        )
    ...
    def draw_agent_info(self):
        ...
        if self.selected_agent:
            ...
            history = self.selected_agent.memory[-5:]
            for line in history:
    ...
    def get_unvisited_connected_room(self, agent: Agent) -> Optional[str]:
        ...
        connected_rooms = self.door_manager.room_connections.get(
            agent.current_room, set()
        )
        unvisited = [
            room
        ...
        if agent.plan.type == PlanType.GO_TO_CAULDRON:
            ...
            if (agent.x, agent.y) == (self.cauldron.x, self.cauldron.y):
                ...
                if agent.role == Role.WITCH:
                    ...
                    print(
                        f"Agent {agent.id} has deposited their ingredient at the cauldron."
                    )
                else:
                    print(
                        f"Agent {agent.id} is not a witch, cannot deposit ingredient."
                    )
            ...
            if agent.target_x is None and agent.target_y is None and agent.waypoints:
                ...
                if (
                    agent.target_x is not None
                ):  # Only remove waypoint if target was set successfully
            ...
            if agent.plan.type == PlanType.STAY:
                ...
                if agent.current_ingredient is None:
                    agent.current_ingredient = random.choice(
                        self.ingredients_list
                    )  # Choose a random ingredient

                # Update room if agent has moved to a new one
                new_room = self.get_room_at_position(
                    agent.x, agent.y, agent.current_room
                )
                ...
                if (
                    agent.current_room == agent.plan.target
                    and not agent.waypoints
                    and agent.target_x is None
                    and agent.target_y is None
                ):
                    ...
                    agent.current_ingredient = (
                        None  # Clear the ingredient after reaching the room
                    )
        ...
        for room_id, room in self.rooms.items():
            ...
            for x, y in room.cells:
                if room_id in self.textures and self.textures[room_id]:
                    ...
                    self.screen.blit(texture, (x * self.CELL_SIZE, y * self.CELL_SIZE))
                else:
        ...
        for agent in self.agents:
            if agent.target_x is not None and agent.target_y is not None:
                if agent.plan and agent.plan.type == PlanType.GOTO_ROOM:
                    if (
                        agent.current_ingredient
                    ):  # Only render if there's a current ingredient
                        ingredient_sprite = self.ingredients_sprites[
                            agent.current_ingredient
                        ]
                        self.screen.blit(
                            ingredient_sprite,
    ...
    async def voting_phase(self, agents):
        ...
        if votes:
            ...
            print(
                f"Agent {most_voted_agent} has been accused with {votes[most_voted_agent]} votes."
            )
    ...
    def handle_vote_outcome(self, agent_id):
        ...
        agent_to_remove = next(
            (agent for agent in self.agents if agent.id == agent_id), None
        )
        if agent_to_remove:
    ...
    async def run(self):
        ...
        while running:
            ...
            if not self.paused:
                ...
                if (
                    self.cauldron.has_enough_ingredients(3)
                    and not voting_phase_occurred
                ):
                    # Trigger the voting phase
                    await self.voting_phase(self.agents)
                    # Debugging statement
                    print("Resetting cauldron ingredients to 0.")
                    self.cauldron.ingredients = 0  # Reset the counter
                    voting_phase_occurred = (
                        True  # Set the flag to indicate voting has occurred
                    )
                else:
                    voting_phase_occurred = (
                        False  # Reset the flag if not enough ingredients
                    )
    ...
    def get_room_at_position(
        self, x: float, y: float, current_room: Optional[str] = None
    ) -> Optional[str]:
        ...
        if current_room:
            ...
            for door in doors:
                ...
                if door_pos and (cell_x, cell_y) == door_pos:
                    ...
                    # Get the other room
                    other_room = (
                        door.room2 if current_room == door.room1 else door.room1
                    )
    ...
    def find_path_through_doors(
        self, agent: Agent, target_x: int, target_y: int
    ...
    def draw_agent_conversation(self, agent):
        ...
        header_text = font.render(
            f"Agent {agent.id}'s Conversation:", True, (255, 255, 255)
        )
